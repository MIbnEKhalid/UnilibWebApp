 ...
 * Usage: node scripts/convertPageImages.js <pdfPath> <pageNumber> <outputFormat> [outputPath]
 * Example: node scripts/convertPageImages.js ./document.pdf 1 png ./output.png
 *
...
async function convertPdfPageToImage(
  ...
  try {
    ...
    if (!pageFound) {
      throw new Error(
        `Page ${pageNum} does not exist in the PDF. Please check the page number.`
      );
    }
  } catch (error) {
...
if (args.length < 3) {
  ...
  console.log("  node scripts/convertPageImages.js ./document.pdf 1 png");
  console.log("  node scripts/convertPageImages.js ./document.pdf 2 jpg ./output.jpg");
  console.log("  node scripts/convertPageImages.js ./book.pdf 5 webp ./page5.webp");
  process.exit(1);
}
